      ...
      <div class="main-panel">
        <div class="content-wrapper">
            ...
            <div class="col-md-12 grid-margin">
              <div class="row">
                <div class="col-12 col-xl-8 mb-4 mb-xl-0">
                  <h2 class="font-weight-bold">Shared with me</h2>
                </div>
              </div>
            </div>
          </div>
          ...
          while($notes = $stmt->fetch(PDO::FETCH_ASSOC)) {
            ...
            $search->execute([$notes['note_id']]);
            while($shared_notes = $search->fetch(PDO::FETCH_ASSOC)) {
              array_push($result, $shared_notes);
            }
          }

          $sql = "SELECT * FROM notes
              WHERE is_public = ? AND NOT owner = ?";
          $stmt = $db->prepare($sql);
          ...
          if(count($result) == 0) {
            echo "No one has shared notes with you";
            return;
          } else {
